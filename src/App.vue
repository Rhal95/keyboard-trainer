...
<template>
  <main>
    ...
    <ul>
      <li v-for="word in game.words.filter(w=>!game.completedWords.includes(w))" :class="classOfWord(word)"><WordCard :word="word" :currentInput="inputContent"/></li>
    </ul>
    <input ref="inputRef" type="text" :value="inputContent" @input="e=>{inputContent = e.target.value.trimStart(); game.checkWord(inputContent);}">
    <ul>
      <li v-for="word in game.words.filter(w=>game.completedWords.includes(w))" :class="classOfWord(word)"><WordCard :word="word" :currentInput="inputContent"/></li>
    </ul>
  </main>
